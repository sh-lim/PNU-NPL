...
*/
	...
	for (int imult=0; imult<nmult; imult++){
		for (int ipt=0; ipt<npt; ipt++){
			...
			h2d_same_mid[imult][ipt]->RebinY(3);
			h2d_mixed_mid[imult][ipt]->RebinY(3);

			float ddphi_mid = h2d_same_mid[imult][ipt]->GetXaxis()->GetBinWidth(1);
			...
			h1d_same_dphi_long_mid[imult][ipt][0]->Scale(ddeta_mid/6.0);
			...
			h1d_same_dphi_short_mid[imult][ipt][0]->Scale(ddeta_mid/2.0);
...
//	h1d_Yassociated_mult_mid[1][npt]=h1d_Yassociated_ds;
...
#if 1
	cfig1[i] = new TCanvas(cfigNames1[i].c_str(), cfigNames1[i].c_str(), 1.1*300*4, 300*3);
	cfig1[i]->Divide(4,3);
	...
	cfig2[i] = new TCanvas(cfigNames2[i].c_str(), cfigNames2[i].c_str(), 1.1*300*4, 300*3);
	cfig2[i]->Divide(4,3);
