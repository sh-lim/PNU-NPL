...
//	const float multbin[nmult+1] = {};
	...
	TH2D *h2d_same_LM, *h2d_same_HM, *h2d_mixed_LM, *h2d_mixed_HM;
	...
	for (int imult=0; imult<nmult; imult++){
		for (int ipt=0; ipt<npt; ipt++){
			...
			if(imult==0){
				h2d_same_LM = new TH2D(*h2d_same_mid[imult][ipt]);
				h2d_same_LM->SetName("h2d_same_LM");
				h2d_mixed_LM = new TH2D(*h2d_mixed_mid[imult][ipt]);
				h2d_mixed_LM->SetName("h2d_mixed_LM");
				}
			else if(imult==8){
				h2d_same_HM = new TH2D(*h2d_same_mid[imult][ipt]);
				h2d_same_HM->SetName("h2d_same_HM");
				h2d_mixed_HM = new TH2D(*h2d_mixed_mid[imult][ipt]);
				h2d_mixed_HM->SetName("h2d_mixed_HM");
			}else if(imult<=3&&imult!=0){
				h2d_same_LM->Add(h2d_same_mid[imult][ipt]);
				h2d_mixed_LM->Add(h2d_mixed_mid[imult][ipt]);

			}else if(imult>8){
				h2d_same_HM->Add(h2d_same_mid[imult][ipt]);
				h2d_mixed_HM->Add(h2d_mixed_mid[imult][ipt]);
			}
	...
	h2d_same_mid[0][0]=h2d_same_LM;
	h2d_mixed_mid[0][0]=h2d_mixed_LM;
	h2d_same_mid[1][0]=h2d_same_HM;
	h2d_mixed_mid[1][0]=h2d_mixed_HM;
	...
	//		nnorm_mixed_mid /= h2d_mixed_LM->GetXaxis()->GetBinWidth(1);
	//		nnorm_mixed_mid /= h2d_mixed_LM->GetYaxis()->GetBinWidth(1);
			...
			TH1D *htmp_same_mid = (TH1D*)h2d_same_mid[imult][ipt]->ProjectionX(Form("h1d_dphi_same_mid_mult%02d_pt%02d_1",imult,ipt),etabin_min_mid,etabin_max_mid);
			...
			h1d_dphi_same_mid[imult][ipt]->Add(htmp_same_mid);
			...
			h1d_dphi_same_mid[imult][ipt]->Scale(1./ntrig_mid);
			h1d_dphi_mixed_mid[imult][ipt]->Scale(1./ntrig_mixed_mid);
			h1d_dphi_same_mid[imult][ipt]->Divide(h1d_dphi_mixed_mid[imult][ipt]);
			h1d_dphi_same_mid[imult][ipt]->Scale(nnorm_mixed_mid);
			...
			h1d_dphi_zyam_mid[imult][ipt] = (TH1D*)h1d_dphi_same_mid[imult][ipt]->Clone(Form("h1d_dphi_zyam_mid_mult%02d_pt%02d",imult,ipt));
			...
			cout << imult << " " << ipt << " " << Y_associated_mid << endl; 
	...
	for (int imult=0; imult<nmult; imult++){
		for (int ipt=0; ipt<npt; ipt++){
			...
			float ymax_mid = h1d_dphi_same_mid[imult][ipt]->GetMaximum();
			float ymin_mid = h1d_dphi_same_mid[imult][ipt]->GetMinimum();
			...
			h1d_dphi_same_mid[imult][ipt]->SetMarkerStyle(24);
			h1d_dphi_same_mid[imult][ipt]->SetMarkerSize(1.0);
			h1d_dphi_same_mid[imult][ipt]->Draw("same");
